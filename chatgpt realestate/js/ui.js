...
const GameUI = {
  ...
  updateHUD() {
    ...
    if (!document.getElementById('hud-feed').textContent) {
      this.setHudFeed('▶ Ready', 'neutral');
    }
  },

  setHudFeed(text, tone) {
    var el = document.getElementById('hud-feed');
    if (!el) return;
    el.textContent = text || '';
    if (tone === 'negative') el.style.color = '#FFD2D2';
    else if (tone === 'positive') el.style.color = '#D6FFD5';
    else el.style.color = '#F7E6BF';
  },
  ...
  showMonthResults(results) {
    ...
    if (results.eraChange) {
      ...
      this.setHudFeed(era.icon + ' New Era: ' + era.name, 'positive');
    }

    // Show disasters/events in compact HUD feed (replaces popups)
    if (hasDisasters) {
      var d = results.disasters[0];
      this.setHudFeed(
        d.disaster.icon + ' ' + d.disaster.name + ': ' + (d.coveredAmount > 0 ? 'You paid ' + GameData.formatMoney(d.actualDamage) : 'Repair ' + GameData.formatMoney(d.actualDamage)),
        'negative'
      );
    } else if (hasEvents) {
      var e = results.events[0];
      var effectClass = e.event.effect.value < 0 ? 'negative' : 'positive';
      this.setHudFeed(e.event.icon + ' ' + e.event.title + ' — ' + e.event.description.replace('{city}', e.cityName), effectClass);
    }
    ...
    if (!isAutoPlaying || speed === 1) {
      if (!hasDisasters && !hasEvents && !(results.milestones && results.milestones.length)) {
        ...
        this.setHudFeed('Month: +' + GameData.formatMoney(results.rentIncome) + ' rent / -' + GameData.formatMoney(results.expenses) + ' costs', net >= 0 ? 'positive' : 'negative');
  ...
  renderBank() {
    ...
    html += '<div class="finance-section">' +
      '<div class="finance-section-title">📈 Investments (Stocks, Bonds & Commodities)</div>';
    ...
    available.forEach(function(inv) {
      var typeBadge = inv.type === 'bond' ? '🔵 Bond' : (inv.type === 'commodity' ? '🟠 Commodity' : '🟢 Stock');
