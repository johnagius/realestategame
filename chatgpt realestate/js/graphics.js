...
const GameGraphics = {
  ...
  renderWorldMap(svgEl) {
    ...
    svgEl.innerHTML =
      this._worldDefs() +
      this._worldAtmosphere() +
      this._worldOcean() +
      this._worldContinents() +
      this._worldRelief() +
      this._worldCityLights() +
      this._worldOceanDetails();
  },
  ...
  _worldDefs() {
    return '<defs>' +
      ...
      '</pattern>' +
      '<pattern id="w-grain" width="120" height="120" patternUnits="userSpaceOnUse">' +
        '<circle cx="15" cy="25" r="1.1" fill="#fff" opacity="0.05"/>' +
        '<circle cx="70" cy="30" r="0.9" fill="#fff" opacity="0.04"/>' +
        '<circle cx="100" cy="80" r="1.2" fill="#fff" opacity="0.05"/>' +
        '<circle cx="45" cy="96" r="1" fill="#fff" opacity="0.03"/>' +
        '<circle cx="85" cy="110" r="0.8" fill="#fff" opacity="0.03"/>' +
      '</pattern>' +
      '<radialGradient id="w-vignette" cx="0.5" cy="0.5" r="0.75">' +
        '<stop offset="65%" stop-color="#000" stop-opacity="0"/>' +
        '<stop offset="100%" stop-color="#04121c" stop-opacity="0.35"/>' +
      '</radialGradient>' +
    '</defs>';
  },
  ...
  _worldAtmosphere() {
    var s = '';
    s += '<rect width="1600" height="800" fill="url(#w-grain)"/>';
    s += '<ellipse cx="680" cy="30" rx="760" ry="120" fill="#bfe8ff" opacity="0.12" filter="url(#w-soft)"/>';
    s += '<ellipse cx="1260" cy="100" rx="420" ry="90" fill="#d7f1ff" opacity="0.08" filter="url(#w-soft)"/>';
    return s;
  },
  ...
  _worldOcean() {
    ...
    s += '</g>';
    s += '<ellipse cx="640" cy="360" rx="460" ry="130" fill="#bde8ff" opacity="0.08" filter="url(#w-soft)"/>';
    s += '<ellipse cx="1130" cy="470" rx="280" ry="100" fill="#d4f2ff" opacity="0.07" filter="url(#w-soft)"/>';
    return s;
  },
  ...
  _worldContinents() {
    ...
    s += '</g>';
    // Edge vignette for premium atlas look
    s += '<rect width="1600" height="800" fill="url(#w-vignette)"/>';
  ...
  _worldRelief() {
    var s = '';
    // Soft continental relief and coastline shading for depth
    s += '<g opacity="0.18">';
    s += '<path d="M95,120 C140,90 260,85 345,120 C418,150 442,225 408,300" fill="none" stroke="#264B2F" stroke-width="18" stroke-linecap="round"/>';
    s += '<path d="M330,438 C380,450 420,520 400,620" fill="none" stroke="#2E5A36" stroke-width="16" stroke-linecap="round"/>';
    s += '<path d="M620,105 C700,90 775,112 785,160" fill="none" stroke="#2D5835" stroke-width="12" stroke-linecap="round"/>';
    s += '<path d="M656,250 C740,240 802,325 770,505" fill="none" stroke="#2F5D38" stroke-width="14" stroke-linecap="round"/>';
    s += '<path d="M772,65 C930,40 1160,58 1248,130" fill="none" stroke="#2F5A35" stroke-width="15" stroke-linecap="round"/>';
    s += '<path d="M1028,182 C1112,178 1160,252 1122,316" fill="none" stroke="#2F5A35" stroke-width="10" stroke-linecap="round"/>';
    s += '<path d="M1158,518 C1230,500 1308,535 1310,620" fill="none" stroke="#2F5A35" stroke-width="12" stroke-linecap="round"/>';
    s += '</g>';

    // mountain belts to improve realism
    s += '<g stroke="#6F6658" stroke-width="1.5" opacity="0.35" fill="none">';
    s += '<path d="M120,188 Q150,150 190,174 Q215,188 240,180"/>'; // Rockies
    s += '<path d="M296,472 Q320,430 342,488 Q355,520 350,560"/>'; // Andes
    s += '<path d="M676,144 Q700,122 728,140"/>'; // Alps
    s += '<path d="M846,62 Q865,38 882,60"/>'; // Urals
    s += '<path d="M925,166 Q955,132 998,156"/>'; // Himalaya
    s += '<path d="M752,438 Q770,410 787,435"/>'; // East Africa
    s += '</g>';
  ...
  _worldCityLights() {
    var hotspots = [
      [242,184,5.2],[598,115,4.2],[692,150,4.2],[1220,172,4.8],[1110,248,4.5],[1058,226,4.0],
      [950,340,4.2],[1098,378,3.8],[1215,580,3.8],[292,235,3.8],[312,520,3.6],[720,430,3.8]
    ];
    var s = '<g>';
    hotspots.forEach(function(h) {
      s += '<circle cx="'+h[0]+'" cy="'+h[1]+'" r="'+(h[2]*2.8)+'" fill="#A3D8FF" opacity="0.12" filter="url(#w-glow)"/>';
      s += '<circle cx="'+h[0]+'" cy="'+h[1]+'" r="'+h[2]+'" fill="#E6F6FF" opacity="0.28"/>';
      s += '<circle cx="'+h[0]+'" cy="'+h[1]+'" r="'+(h[2]*0.45)+'" fill="#FFFFFF" opacity="0.75"/>';
    });
    s += '</g>';
    return s;
  },
