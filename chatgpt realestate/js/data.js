...
const GameData = {
  ...
  getEraForYear(year) {
    return this.eras.find(e => year >= e.years[0] && year <= e.years[1]) || this.eras[0];
  },

  getEraRentBoost(year) {
    const era = this.getEraForYear(year);
    const m = era.propertyMultiplier || 1;
    return Math.min(3.5, Math.max(1.15, 0.45 / m));
  },

  getEraCostFactor(year) {
    const era = this.getEraForYear(year);
    const m = era.propertyMultiplier || 1;
    return Math.min(1, Math.max(0.2, m * 2));
  },
  ...
  generateProperty(cityId, type) {
    ...
    const currentEra = this.getEraForYear(year);
    ...
    const price = Math.max(500, Math.round(basePrice * city.priceMultiplier * eraMultiplier / 20 / 10) * 10);
    ...
    const annualRent = price * city.rentYield * rentMultiplier * condPenalty * this.getEraRentBoost(year);
    const monthlyRent = Math.round(annualRent / 12);

    // Maintenance
    const annualMaintenance = price * typeDef.maintenanceRate * 0.45;
    const monthlyMaintenance = Math.round(annualMaintenance / 12);

    // Licensing fee
    const annualLicense = Math.max(12, (this.licensingFees[type] || 500) * this.getEraCostFactor(year) * 0.2);
